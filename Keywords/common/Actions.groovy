...
public class Actions {
	...
	def void NavigateByTabMenusNewChanges(String tabName, String menuItems) {
		...
		if(!WebUI.verifyElementVisible(tab, FailureHandling.OPTIONAL)){
			Click(findTestObject('Common/pge_Common/wel_TabsMoreOptions'))
			def menu = ObjectRepository.findTestObject('Common/pge_Common/wel_MenuItem_New', [('item') : tabName])
			WebUI.mouseOver(menu)
			Thread.sleep(GlobalVariable.gSleepTime)
		}
		...
		def len = items.length
		for (def i=0; i<len - 1; i++ ) {
			def menu = ObjectRepository.findTestObject('Common/pge_Common/wel_MenuItem_New', [('item') : items[i]])
			Thread.sleep(GlobalVariable.gSleepTime)
			WebUI.mouseOver(menu)
		}
		def lastmenu = ObjectRepository.findTestObject('Common/pge_Common/wel_MenuItem_New', [('item') : items[len-1]])
		Click(lastmenu)
	}
	...
	def void SelectRadioByLabelValue(String label,String value){
		...
		def tab = ObjectRepository.findTestObject('Policy/Legacy/pge_NewSubmissions/wrg_Generic_Radio', [('label'): label, ('value') : value ] )
	...
	def void SelectTableItemByRefColumn(TestObject TableObject,String RefColHeader,String RefColValue,String ActionColHeader){
		...
		//def TableObject = ObjectRepository.findTestObject('Policy/pge_AccountSummary/wtb_OpenTransactions')
	...
	//		TestObject tab = ObjectRepository.findTestObject("Policy/pge_JobWizard/pge_Locations/btn_SelectLocation", [('Number') : LocationNumber])
	//		Click(tab)
	//	}
	...
	//			TestObject tab = ObjectRepository.findTestObject("Policy/pge_JobWizard/pge_Payment/wel_Select", [('InstallmentPlan') : InstallmentPlan])
	//		Click(tab)
	//	}
	//
	//	@Keyword
	//	def void SelectState(String State) {
	//		TestObject tab = ObjectRepository.findTestObject("Policy/pge_JobWizard/pge_WCCoverages/wel_State", [('State') : State])
	//		Click(tab)
	//	}
	//	@Keyword
