...
public class Assertion {
						...
						try{
							if(WebUI.verifyElementPresent(NextPage, 1, FailureHandling.OPTIONAL)){
								if(WebUI.verifyElementClickable(NextPage, FailureHandling.OPTIONAL) ) {
									...
									Thread.sleep(GlobalVariable.pWaitTimeOut)
	...
	def VerifyText(TestObject to, String text ) {
		def flag
		try {
			WebUI.verifyElementPresent(to, GlobalVariable.pWaitTimeOut, FailureHandling.STOP_ON_FAILURE)
			WebUI.waitForPageLoad(GlobalVariable.pWaitTimeOut)
			flag = WebUI.verifyElementText(to, text)
		} catch (Exception e) {
			//WebUI.sendKeys(to, Keys.chord(Keys.F5))
			Thread.sleep(GlobalVariable.pSleepTime)
			Thread.sleep(GlobalVariable.pSleepTime)
			WebUI.waitForPageLoad(GlobalVariable.pWaitTimeOut)
			flag = WebUI.verifyElementText(to, text)
			e.printStackTrace()
